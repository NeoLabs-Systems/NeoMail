...
function saveLabel(emailId, userId, label) {
  db.prepare('UPDATE emails SET ai_label = ?, ai_label_done = 1 WHERE id = ?').run(label, emailId);
  const row = db.prepare('SELECT id FROM labels WHERE user_id = ? AND LOWER(name) = LOWER(?)').get(userId, label);
  if (row) db.prepare('INSERT OR IGNORE INTO email_labels (email_id, label_id) VALUES (?, ?)').run(emailId, row.id);
}

router.post('/smart-replies/:emailId', async (req, res) => {
  ...
  try {
    const replies = await ai.smartReplies(email.subject, email.body_html, email.body_text);
...
router.post('/summarize-attachment/:emailId/:attId', async (req, res) => {
  if (!ai.isAvailable()) return res.status(503).json({ error: 'AI not configured' });
  const email = db.prepare(`
    SELECT e.subject, e.body_text FROM emails e JOIN accounts a ON a.id = e.account_id
    WHERE e.id = ? AND a.user_id = ?
  `).get(req.params.emailId, req.session.userId);
  if (!email) return res.status(404).json({ error: 'Email not found' });
  const att = db.prepare(
    'SELECT id, filename FROM email_attachments WHERE id = ? AND email_id = ?'
  ).get(req.params.attId, req.params.emailId);
  if (!att) return res.status(404).json({ error: 'Attachment not found' });
  try {
    const summary = await ai.summarizeAttachment(att.filename, email.subject, email.body_text);
...
router.post('/label/:emailId', async (req, res) => {
  ...
  if (label) saveLabel(email.id, req.session.userId, label);
...
router.post('/cursor-rules/apply/:emailId', async (req, res) => {
  ...
  if (actions.label) saveLabel(email.id, req.session.userId, actions.label);
