...
router.get('/', (req, res) => {
  ...
  if (folder === 'awaiting') {
    ...
    sql += ' AND e.snoozed_until IS NOT NULL AND e.snoozed_until > unixepoch() AND e.is_trash = 0';
  } else {
    sql += ' AND e.folder = ? AND e.is_archived = 0 AND e.is_trash = 0 AND (e.snoozed_until IS NULL OR e.snoozed_until <= unixepoch())';
    params.push(folder);
  }
  ...
  if (label) {
    sql += ` AND (
      ...
      OR LOWER(e.ai_label) = LOWER(?)
    )`;
    params.push(label, req.session.userId, label);
  }
...
router.get('/scheduled', (req, res) => {
  const rows = db.prepare(
    'SELECT id, account_id, send_at, to_addr, subject, created_at FROM scheduled_emails WHERE user_id = ? ORDER BY send_at ASC'
  ).all(req.session.userId);
  res.json(rows);
});

router.delete('/scheduled/:id', (req, res) => {
  db.prepare('DELETE FROM scheduled_emails WHERE id = ? AND user_id = ?').run(req.params.id, req.session.userId);
  res.json({ ok: true });
});
...
router.patch('/:id', (req, res) => {
  ...
  if (updates.is_archived === 1) {
    ...
    db.prepare('UPDATE emails SET folder = ? WHERE id = ?').run('INBOX', email.id);
  }
  if (updates.is_trash === 0) {
    db.prepare('UPDATE emails SET folder = ? WHERE id = ?').run('INBOX', email.id);
  }
...
router.post('/schedule', async (req, res) => {
  const { account_id, send_at, to, cc, bcc, subject, html, text, in_reply_to } = req.body;
  if (!account_id || !send_at || !to || !subject) return res.status(400).json({ error: 'account_id, send_at, to, subject required' });
  const sendTs = Number(send_at);
  if (!Number.isInteger(sendTs) || sendTs <= Math.floor(Date.now() / 1000)) {
    return res.status(400).json({ error: 'send_at must be a future unix timestamp' });
  }
  const account = db.prepare('SELECT * FROM accounts WHERE id = ? AND user_id = ?').get(account_id, req.session.userId);
  if (!account) return res.status(404).json({ error: 'Account not found' });
  const result = db.prepare(`
    INSERT INTO scheduled_emails (user_id, account_id, send_at, to_addr, cc_addr, bcc_addr, subject, body_html, body_text, in_reply_to)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(req.session.userId, account_id, sendTs,
    String(to).slice(0, 2000), cc ? String(cc).slice(0, 2000) : null,
    bcc ? String(bcc).slice(0, 2000) : null, String(subject).slice(0, 1000),
    html || null, text || null, in_reply_to || null);
...
router.patch('/:id/snooze', (req, res) => {
  ...
  if (!Number.isInteger(until) || until <= Math.floor(Date.now() / 1000)) {
    return res.status(400).json({ error: 'until must be a future unix timestamp' });
  }
  db.prepare('UPDATE emails SET snoozed_until = ?, is_read = 0 WHERE id = ?').run(until, email.id);
  res.json({ ok: true });
});
...
router.post('/:id/unsnooze', (req, res) => {
  const email = getUserEmail(req.session.userId, req.params.id);
  if (!email) return res.status(404).json({ error: 'Not found' });
  db.prepare('UPDATE emails SET snoozed_until = NULL WHERE id = ?').run(email.id);
  res.json({ ok: true });
});
...
router.post('/archive-all', (req, res) => {
  ...
  let sql = `
    UPDATE emails SET is_archived = 1
    WHERE id IN (
      SELECT e.id FROM emails e
      JOIN accounts a ON a.id = e.account_id
      WHERE a.user_id = ? AND e.is_trash = 0 AND e.is_archived = 0
  `;
  const params = [userId];
  if (folder && typeof folder === 'string') {
    sql += ' AND e.folder = ?';
    params.push(folder.slice(0, 200));
  }
  sql += ')';
