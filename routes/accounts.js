  ...
  try {
    ...
    startIdleWatcher(account, async (acc) => {
      ...
      db.prepare('INSERT INTO notifications (user_id, account_id, type, payload) VALUES (?, ?, ?, ?)')
        .run(req.session.userId, acc.id, 'new_mail', JSON.stringify({
          accountId:  acc.id,
...
router.put('/:id', (req, res) => {
  ...
  const updatedUserId = req.session.userId;
  startIdleWatcher(updatedAccount, async (acc) => {
    const latest = db.prepare(
      'SELECT subject, from_name, from_email FROM emails WHERE account_id = ? ORDER BY id DESC LIMIT 1'
    ).get(acc.id);
    db.prepare('INSERT INTO notifications (user_id, account_id, type, payload) VALUES (?, ?, ?, ?)')
      .run(updatedUserId, acc.id, 'new_mail', JSON.stringify({
        accountId:  acc.id,
        subject:    latest?.subject    || null,
        from_name:  latest?.from_name  || null,
        from_email: latest?.from_email || null,
