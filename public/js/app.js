...
async function openEmail(email) {
  ...
  document.getElementById('view-from-email').textContent = `<${data.from_email}>`;
  ...
  const iframeDoc = `<!DOCTYPE html><html><head>
    ...
    <style>
      *, *::before, *::after { box-sizing: border-box; }
      ...
      td, th { padding: 8px 12px; border: 1px solid #e5e5ea; }
  ...
  if (data.attachments?.length) {
    attList.innerHTML = data.attachments.map(att => `
      <div class="attach-chip" style="cursor:pointer"
...
document.addEventListener('keydown', async (e) => {
  ...
  if (document.getElementById('compose-overlay')?.style.display === 'flex') return;
